...
def create_graph_for_language(translations_manager, lang, output_directory, base_name, notifier):
    notifier.notify('create graph for language "' + lang + '" in directory "' + output_directory + '"')
...
if(args.lang != 'all'):
    if(args.lang in translations_manager.get_languages()):
        create_graph_for_language(translations_manager, args.lang, args.output_directory, args.base_name, notifier)
    else:
        print(args.lang + ' unknown, look at the ' + args.yaml_directory + ' file')
else:
    for lang in translations_manager.get_languages():
        create_graph_for_language(translations_manager, lang, args.output_directory, args.base_name, notifier)
